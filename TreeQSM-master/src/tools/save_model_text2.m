...
function save_model_text(QSM,string, path)
...
str = [path, 'results/cyl_data_',string,'.txt'];
str = strjoin(str, "");
fid = fopen(str, 'wt+');
fprintf(fid, [repmat('%g\t', 1, size(CylData,2)-1) '%g\n'], CylData.');
fclose(fid);

str = [path, 'results/branch_data_',string,'.txt'];
str = strjoin(str, "");
fid = fopen(str, 'wt');
fprintf(fid, [repmat('%g\t', 1, size(BranchData,2)-1) '%g\n'], BranchData.');
fclose(fid);

str = [path, 'results/tree_data_',string,'.txt'];
str = strjoin(str, "");
fid = fopen(str, 'wt');
